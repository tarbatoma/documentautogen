...
const DocumentsPage = () => {
  ...
  const handleDownload = async (document) => {
    try {
      if (!document.file_path) {
        toast.error('Documentul nu a fost încă generat');
        return;
      }
      ...
      const { data, error } = await supabase
        .storage
        .from('documents')
        .download(`${user.id}/${document.file_path}`);
                      ...
                      <tr key={doc.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <div className="flex justify-end space-x-2">
                            ...
                            >
                              <Download className={`w-5 h-5 ${!doc.file_path ? 'opacity-50' : ''}`} />
                            </button>
                            <button
                              ...
                              disabled={!doc.file_path}
                            >
                              <Send className={`w-5 h-5 ${!doc.file_path ? 'opacity-50' : ''}`} />
                            </button>
                            <button
                              ...
                              disabled={!doc.file_path}
                            >
                              <Eye className={`w-5 h-5 ${!doc.file_path ? 'opacity-50' : ''}`} />
                            </button>
                            <button
